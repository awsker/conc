...
namespace conc.game.math
{
    public class RectangleF : IPolygon
    {
        ...
        {
            float minx = Math.Min(line.Start.X, line.Target.X);
            float miny = Math.Min(line.Start.Y, line.Target.Y);
            float maxx = Math.Max(line.Start.X, line.Target.X);
            float maxy = Math.Max(line.Start.Y, line.Target.Y);
            if (line.LineType != LineType.BothCapped)
            {
                if (line.LineType == LineType.Infinite || line.Target.X < line.Start.X)
                    minx = float.NegativeInfinity;
                if (line.LineType == LineType.Infinite || line.Target.X > line.Start.X)
                    maxx = float.PositiveInfinity;
                if (line.LineType == LineType.Infinite || line.Target.Y < line.Start.Y)
